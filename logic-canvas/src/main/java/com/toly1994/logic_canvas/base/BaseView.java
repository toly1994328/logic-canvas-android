...
public abstract class BaseView extends View {
    ...
    protected Pos mWinSize = pos.clone(
            CanvasUtils.getScreenWidth(getContext()),
            CanvasUtils.getScreenHeight(getContext()));
    ...
    public void parseEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > dp2px(8f)) {
                    isToLeft = dx < 0;
                    isToLR = true;
                } else {
                    isToDown = dy > 0;
